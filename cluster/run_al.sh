#!/bin/bash
# The program HTCondor runs INSIDE the docker container for the active-learning
# run. Same environment as run_gpu.sh (the uv env + Hugging Face cache that
# cluster/setup_env.sh prepared once in your mounted home directory); it just
# runs the AL benchmark instead of the training experiment.
set -euo pipefail
...
# repeatedly allocates/frees TabPFN activations of varying size.
...
PYTHONPATH=src PYTHONUNBUFFERED=1 .venv/bin/python src/scripts/run_active_learning_gpu.py
